...
export const VALID_SYNTAXES: SrcSyntax[] = ['regex', 'path-to-regexp', 'equals'];
export const REDIRECT_STATUS_CODES = [301, 302, 307, 308];
export const VALID_ACTION_TYPES = ['rewrite', 'redirect', 'set-status'] as const;
...
export function extractTransformFlags(
  ...
  return {
    ...
    appendRequestQuery: flags['--append-request-query'] as
      | string[]
      | undefined,
    deleteRequestQuery: flags['--delete-request-query'] as
      | string[]
      | undefined,
  };
}
 ...
 */
export function hasAnyTransformFlags(
  flags: Record<string, unknown>
): boolean {
...
/**
 * Computes the `continue` field for a route based on its configuration.
 * Routes that only set response headers (without terminating) should continue
 * processing so subsequent routes can also match.
 */
export function computeShouldContinue(
  dest: string | undefined,
  status: number | undefined,
  headers: Record<string, string>,
  transforms: Transform[]
): boolean {
  const isRedirect = dest && status && REDIRECT_STATUS_CODES.includes(status);
  const isTerminating = isRedirect || (status && !dest);
  ...
  return hasAnyResponseHeaderMutation && !isTerminating;
}

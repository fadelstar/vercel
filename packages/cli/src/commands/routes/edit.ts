...
function applyFlagMutations(
  ...
  if (flags['--syntax'] !== undefined) {
    const syntax = flags['--syntax'] as string;
    ...
    if (hasConds.length > 0 || missingConds.length > 0) {
      ...
      if (hasConds.length > 0) {
        ...
        hasConds.forEach((c, i) => {
          output.print(`    ${chalk.gray(`${i + 1}.`)} ${formatConditionDisplay(c)}\n`);
        });
      }
      if (missingConds.length > 0) {
...
export default async function edit(client: Client, argv: string[]) {
  ...
  // Recompute `continue` field
  const dest = route.route.dest;
  const status = route.route.status;
  const finalHeaders = route.route.headers ?? {};
  const finalTransforms = ((route.route as any).transforms ?? []) as Transform[];
  ...
  if (shouldContinue) {
    (route.route as any).continue = true;
  } else {
    delete (route.route as any).continue;
  }

...
describe('routes add', () => {
  ...
  describe('continue field behavior', () => {
    // The `continue` field should be auto-set when:
    // - Route has response headers AND
    // - Route is NOT a redirect
    // This matches the front-end behavior in form-state.ts

    it('should set continue:true for header-only routes', async () => {
      let capturedBody: unknown;
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const body = capturedBody as any;
      expect(body.route.route.headers).toEqual({
        'Cache-Control': 'public, max-age=3600',
      });
    ...
    it('should set continue:true for rewrite with headers', async () => {
      let capturedBody: unknown;
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const body = capturedBody as any;
      expect(body.route.route.dest).toBe('https://api.example.com');
      expect(body.route.route.headers).toEqual({ 'X-Custom': 'value' });
      // Rewrite with headers should have continue:true (matching front-end)
      expect(body.route.route.continue).toBe(true);
    });

    it('should NOT set continue for redirect with headers', async () => {
      let capturedBody: unknown;
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const body = capturedBody as any;
      expect(body.route.route.dest).toBe('/new-path');
      expect(body.route.route.status).toBe(301);
      expect(body.route.route.headers).toEqual({
        'X-Redirect-Reason': 'moved',
      });
      // Redirects should NOT have continue set
      expect(body.route.route.continue).toBeUndefined();
    });

    it('should NOT set continue for routes without headers', async () => {
      let capturedBody: unknown;
      ...
      const body = capturedBody as any;
      expect(body.route.route.dest).toBe('https://api.example.com');
      expect(body.route.route.headers).toBeUndefined();
      // No headers = no continue
      expect(body.route.route.continue).toBeUndefined();
    });

    it('should set continue:true for response header transform-only routes', async () => {
      // Routes that only use --append-response-header or --delete-response-header
      // should also get continue:true since they only modify headers
      let capturedBody: unknown;
      ...
      expect(body.route.route.headers).toBeUndefined();
      // Transform-only routes with response header ops should continue
      expect(body.route.route.continue).toBe(true);
    });

    it('should NOT set continue for status-only routes with headers', async () => {
      // Status-only routes (like 403 blocking) should terminate and not continue
      // even if they have headers, to avoid subsequent routes overriding the status
      let capturedBody: unknown;
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const body = capturedBody as any;
      expect(body.route.route.status).toBe(403);
      expect(body.route.route.headers).toEqual({ 'X-Blocked': 'true' });
      expect(body.route.route.dest).toBeUndefined();
      // Status-only routes should NOT have continue, even with headers
      expect(body.route.route.continue).toBeUndefined();
    });
  });
